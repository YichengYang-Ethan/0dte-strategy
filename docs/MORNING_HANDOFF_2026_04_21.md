...
intraday pin。真正的决策是 A2 (继续做 0DTE, Joey-style feature engineering)
...
| V2 | Joey +378% median winner real | Joey 数字诚实 |
...
### A2 — Joey-style signal engineering (keep working on 0DTE)
...
- Exit at −40% stop 或 EOD (V2 validated Joey params)
- **优点**：和 Joey 同赛道、有实盘潜力、复用全部 infrastructure
- **缺点**：Round 5 warning "manufactured alpha" 风险；没有 massive flow 数据很难超越 Joey
...
- V2 证实 Joey +378% winner 可达 → payoff 几何成立
- check0 3.3% unconditional hit rate + Joey-style sizing/stop 在数学上可盈利
- 全部 infrastructure (leak_safe, 952-day data, 4-tier OOS) 已就位

**A2 风险点**：
- Round 5 明确警告 "feature search = manufactured alpha"
- 无 massive net flow → 与 Joey 关键 alpha 源有 gap
...
### Scripts（可重跑）
```
scripts/validate_v2_joey_payoff_empirical.py      # Joey +300% 实证验证
...
### Docs
```
docs/joey_bot_extracted_specs.md                   # 18 条 Joey intel
docs/joey_payoff_model.py                          # V2-calibrated baseline
docs/strategy_delta_vs_joey.md                     # 5 optimizations (§1/3/4 soft, §5 FALSIFIED)
