...
parallel. These findings reshape `docs/strategy_delta_vs_peer.md` §5 and
partially weaken reference-operator intel #17.
...
### Setup
- **Claim tested:** `docs/strategy_delta_vs_peer.md` §5 says weak-trend
  no-trade is "very high impact free alpha" because the reference operator's BE WR on
  weak-trend is 58.3%.
...
- **Edit `docs/strategy_delta_vs_peer.md` §5**: downgrade from "very
...
- Does not mean the reference operator is wrong about weak-trend being the hardest regime.
  He probably is, because OTM convexity needs movement to print.
- Does not mean we should copy the reference operator and also ignore weak-trend — V4 just
  shows that identifying weak-trend prospectively is harder than
  `strategy_delta_vs_peer.md` §5 assumed.
...
### Setup
- **Claim tested:** reference-operator intel #17 says "no signals in first 15 min after
...
This contradicts the naive reading of the reference operator #17 ("first 15 min is
chaotic, don't trade"). At the option level, the first 15 min is
a *good* execution environment.

### But this is not a full refutation

the reference operator's rule likely isn't about option microstructure — it's about
...
- **Downgrade reference-operator intel #17 in `docs/peer_bot_extracted_specs.md`**:
...
## Updated delta vs the reference operator
...
| 4 | Trigger density calibration 3-5/day | Strengthened — V3 says first 15 min is tradeable at microstructure level, so filter calibration must address GEX stability, not copy the reference operator's window blindly |
| 5 | Weak-trend no-trade as free alpha | **FALSIFIED with naive 30-min range classifier.** Claim preserved in spirit (the reference operator's weak-trend is real) but demoted from "free" to "requires a classifier we don't yet have." |
...
- If GPT Pro also flags V4-style failure → **convergent evidence**, ship
  update to `strategy_delta_vs_peer.md` §5 as FALSIFIED.
...
## V2 — the reference operator payoff model empirical check: **CONFIRMED with signal-conditionality flag** ✅ ⚠️

### Setup
- **Claim tested:** the reference operator's `+300% typical win / -40% hard stop` numbers,
  encoded in `docs/peer_payoff_model.py` as baseline
- **Test:** 951 days. Pick call with |delta| ≈ 0.20 at 09:45 ET. Enter
  at ask (the reference operator's fill method). Exit at 15:55 bid OR stop-out (whichever
  first) under 4 stop levels: none, −40%, −60%, −100%
- **Critical property:** this is UNCONDITIONAL on any signal — pure
  mechanical baseline. the reference operator's claimed payoff numbers must be at least
...
#### Finding 1 — the reference operator's +300% number is REAL
Unconditional median win on 0.20Δ 0DTE calls held 09:45 → 15:55 is
**+378% gross** under a −40% stop rule (+298% without stop). the reference operator's
"+300% typical win" is not memory survivorship — it's within the
empirical distribution. This **validates `peer_payoff_model.py`'s
squeeze regime number** (+300%) as the unconditional baseline.

#### Finding 2 — the reference operator's −40% stop is mathematically correct
- No stop:  PF **0.97** (mean EV < 0 — unconditional strategy loses money)
- −40% stop: PF **1.28** (+32% improvement)
- −60% stop: PF **1.11**
- −100%/none: equivalent

the reference operator's −40% stop converts an unprofitable mechanical strategy into a
...
currently uses `sl_pct = -0.50` in `BacktestConfig`. the reference operator is at −0.40.
...
#### Finding 3 — the reference operator's 70% WR is impossible as self-reported
Unconditional WR at −40% stop is **9.7%**. If the reference operator's signal produced a
...
### Reshaping `docs/peer_payoff_model.py`

The `win_by_regime` dict in `peer_payoff_model.py` assumed weak-trend
...
- **the reference operator's signal effect:** shifts WR distribution upward (not median
...
Updated `peer_payoff_model.py` `win_by_regime` should be uniform ≈ +378%
across regimes, and `kelly_wr` should be the regime-varying parameter.

### Critical implications for our strategy

1. **Our v5 swing_1dte backtest uses `sl_pct = -0.50`; the reference operator uses −0.40.**
...
- **Queue new test V7: v5 signal under −40% stop** — see if our v5
  backtest's PF 1.23 can be pushed higher just by adopting the reference operator's
...
|---|---|---|
| the reference operator's +300% winner | ✅ REAL, unconditional | V2 empirical median +378% |
| the reference operator's −40% stop | ✅ CORRECT, +32% PF vs no-stop | V2 PF comparison |
| the reference operator's 70% WR | ❌ MATHEMATICALLY IMPOSSIBLE with self-reported payoff | V2 derivation |
| reference-operator intel #17 (open 15min no-trade) | 🟡 No microstructure support; GEX-primitive test pending | V3 |
| Weak-trend free alpha (our §5 claim) | ❌ FALSIFIED (30-min open-range classifier) | V4 |
| Our backtest at `sl_pct = -0.50` vs the reference operator `-0.40` | 🟡 10% tuning difference, test required (V7) | V2 + engine.py read |
...
−31% PF leak** from using no stop loss when the reference operator's −40% is mechanically
...
the reference operator parameters is not a replacement check … because
...
not validate the reference operator or kill R0 — most likely indicate horizon
...
### Q1 — the reference operator claim reliability
...
| "No kill, Kelly enough" | Fact reliable, adequacy claim unreliable | Convergent — the reference operator's adequacy claim is unverified | ✅ Convergent |
...
|---|---|---|
| the reference operator's +300% winner | ✅ REAL, unconditional (V2 beats GPT Pro here) | V2 |
| the reference operator's −40% stop | ✅ Mathematically correct on unconditional path | V2 |
| the reference operator's 70% WR | ❌ IMPOSSIBLE with claimed payoff | V2 + GPT Pro convergent |
| reference-operator intel #17 | 🟡 No microstructure support; GEX-primitive test pending | V3 |
...
- `scripts/validate_v2_payoff_empirical.py` + logs/v2_*
...
- `docs/peer_payoff_model.py` rewritten with V2 empirical baseline
...
- V2 used 09:45 → 15:55 hold (full day): median win +378%, WR 9.7% at
  −40% stop
- check0 restricts to 60-min hold (the reference operator's claimed window): WR drops to 3.26%

**Implication:** the reference operator's full +300% winner size is reachable in the stated
...
binding one. the reference operator's claimed 3-5 trades/day combined with 10-60 min
holds is mathematically consistent with ~3-10% per-trade WR on the
unconditional baseline — not 70%.

### Derived the reference operator signal alpha budget

Unconditional target-hit rate:  3.3% (60-min) / 9.7% (full day)
the reference operator's real live WR (estimate): 20-30% (per V2 derivation)
...
- **Tighten check2/check3 budgets**: if subsequent checks show our
  ...
  not worth the engineering. We need 6-9× lift for reference-class returns.
- **Update `peer_payoff_model.py`**: add `target_hit_rate_60min = 0.033`
...
Our `docs/peer_bot_extracted_specs.md` cited Adams/Fontaine/Ornthanalai
...
- **Do NOT update `peer_bot_extracted_specs.md` to remove the Adams
...
1. **A2 (practitioner-style feature engineering)** — most defensible. No
   specific mechanism story, just ensemble of weak signals with
   strict OOS. Acknowledges honestly that we lack net-flow data
   the reference operator uses and so cannot exceed the reference edge; frames this as
...
| Claim / Item | Verdict | Evidence |
|---|---|---|
| the reference operator's +300% winner | ✅ REAL on full-day hold, 🟡 marginal on 60-min hold | V2 + check0 |
| the reference operator's −40% stop | ✅ Mathematically correct on unconditional path | V2 |
| the reference operator's 70% WR | ❌ IMPOSSIBLE with claimed payoff | V2 + GPT Pro convergent |
| reference-operator intel #17 | 🟡 No microstructure support; GEX-primitive test pending | V3 |
| §5 weak-trend free alpha | ❌ FALSIFIED | V4 + GPT Pro convergent |
...
| R0 signal lift bar | ≥6× unconditional for reference-class returns | V2 + check0 derivation |
...
The A2 thesis ("practitioner-style signal engineering on raw option-structure
...
|---|---|---|
| the reference operator's +300% winner | ✅ REAL on full-day hold | V2 |
| the reference operator's −40% stop | ✅ Mathematically correct | V2 |
| the reference operator's 70% WR | ❌ IMPOSSIBLE with payoff | V2 + GPT Pro R6 |
| reference-operator intel #17 (open filter) | 🟡 No microstructure support | V3 |
