...
New project. Building a fully automated intraday 0DTE options trading bot on SPX (cash-settled European options). A UIUC CS/Stat undergrad friend ("Joey") runs a production version of this exact strategy on moomoo, so I have both (1) a concrete reference architecture to reason about, and (2) his candid chat history describing how it works. This brief is a from-scratch design request — not a validation of existing work.
...
## Production reference — Joey's actual implementation

- **Platform**: local Python + moomoo API (IB does not give him options permissions)
...
### Joey's specific trade example (actual bot output as he described it)
...
### Red flags on Joey's self-report
...
- When asked whether narrative is bot output or his interpretation, he said "bot output" — either he has an LLM layer, or he's retconning

## What I have

- IB paper account with options permissions (full Greeks + OPRA)
- moomoo OpenD (local, port 11111) as backup broker
- Python 3.14 on macOS, prior experience writing auto-execution bots
- Existing research infrastructure from the archived 1DTE project:
...
- Budget: willing to expense $160-360/mo data subscription
- Engineering: 6–12 weeks of evening/weekend time

## What I explicitly do NOT have
...
Is Joey's framing — MMs as profit-maximizing agents that actively push spot toward strikes minimizing their book loss — consistent with modern 0DTE microstructure research? Or is it retail folklore dressed up?
...
Joey names 11 raw features + "aggregated and combined features picked empirically". Without paying for massive or Cboe Open-Close, what features are constructible from Theta Options Pro ($160/mo: every OPRA NBBO quote + every trade with size/exchange/conditions + 1-min Greeks + streaming OI updates)?
...
- What is the minimum feature set to replicate Joey's level of performance (even with 50% haircut)? What features require premium data you cannot substitute?

### Q3. Execution design for intraday 0DTE via IB API

I'm planning to use IB (not moomoo) because I have options permissions there. Specifically for SPX 0DTE:
- Limit order at bid-ask (Joey's style) vs. mid vs. adaptive — what's the correct default for fast markets?
...
Assume:
- Win rate: 55% (haircut from Joey's 70%)
- Win size: +60%
- Loss size: −40%
...
If you were me, what quantitative criteria would cause you to kill this project partway through and revert to a simpler strategy? Be specific: "if paper trade WR < X after N signals, stop" type thresholds.
...
Do not pull punches. I've archived my 1DTE work and committed to the pivot; I need you to stress-test the 0DTE design before I start engineering, not to be polite.
