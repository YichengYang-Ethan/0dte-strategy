...
Thank you for Round 1 and Round 2. I acted on the Day 4 instrument sanity check you prescribed and the results matched your hypothesis — spot has edge, option wrapper eats it. Now I have a completely new set of information from a peer 0DTE operator that changes the decision. I need Round 3.

## What changed

I re-read the reference operator's full field research history end-to-end, with specific attention to execution and infrastructure details I had glossed over. Major corrections to my prior belief about the reference bot:

### 1. the reference operator's bot is **fully automatic**, not discretionary

I had told you the reference bot "outputs signals and signals are clicked manually." That was wrong. Direct quote:

> Me: "so is your bot outputting information or placing trades directly?"
> the reference operator: **"directly trading. connected to the broker API."**
> the reference operator: "yesterday it used market orders and got baited by fake-depth orders, lost me a few hundred bucks. I changed it to limit bid-ask."

He uses **moomoo API** (IB does not give the reference options permissions). The reference bot runs locally (not AWS — "because the bot is still half-finished, I have to watch it"), but watching = monitoring for bugs, not making trading decisions. Trade execution is fully auto.
...
### 3. The reference bot has an exit bug that has not't fixed
...
Critically: **follow MM, do not fight MM**. This is important because it means the reference thesis is **not** "dealer short gamma → price pins at walls" (the traditional Barbon-Buraschi / SpotGamma story). The reference thesis is closer to: "identify the strike where MM inventory forces them to push price, then ride along."
...
If true, this is not a "signal" — it's a gold mine. If it's survivorship / selection bias in memory, it's self-narrative. I cannot distinguish from outside.

### 9. Specific bot output reviewed
...
the reference operator: **"the bot's output."**

Either the reference has a narrative-generation layer on top of numeric signals, or the reference has a rule that outputs strike + direction + justification tag, or retconning is happening. I can't tell.
...
- IB does not give the reference options permissions (so the reference can't use IB for 0DTE)
...
### Path B — Pivot to replicating the reference operator's intraday 0DTE bot
- Upgrade to Theta Options Pro $160/month (intraday 1-min quotes + Greeks + OI)
- Optionally add a paid L2 tick-flow provider (approx $200/mo)/month for live streaming (or stay on Theta streaming and accept reconnects)
- Rewrite the engine from EOD swing to bar-level intraday replay
- Implement the reference operator's architecture: 11 raw features + aggregated + cross features + per-symbol specialization
...
### Q2. Is replicating the reference operator's bot (Path B) feasible in 6–8 weeks for an experienced undergrad?

Specifically:
- Is Theta Options Pro $160/mo sufficient, or do I need massive at $200?
- Is 3 months of historical 1-min SPX 0DTE data enough to avoid overfitting this style of rule-based engine?
...
the reference operator claims 70% WR with convex payoff. I should assume at least 30–50% haircut on self-reported numbers. If true pattern is closer to 55% WR with +80% winners / −40% losers, what does that look like after:
...
- **B** — full pivot to replicating the reference operator's intraday 0DTE bot, ~6–8 weeks
- **C** — A first (2 weeks), B as longer-term project
- **D** — something else entirely (please name it)

### Q5. Red flags in the reference operator's self-report to watch for
...
- If the reference operator uses massive.com's live OI-equivalent (they claim aggressor-signed volume live), do I need that, or is yesterday's OI + today's signed-volume-proxy-from-tick-rule acceptable?
